...
namespace DustInTheWind.BundleWithCustomGui.CustomBootstrapperApplication.Presentation.Commands
{
    public class InstallCommand : ICommand
    {
        ...
        public InstallCommand(IInstallerEngine installerEngine)
        {
        ...
        private void HandleDetectComplete(object sender, DetectEventArgs e)
        {
            dispatcher.Invoke(() =>
            {
                bool isAnyPackageAbsent = e.Packages.Any(x => x.State == PackageState.Absent);

                if (isAnyPackageAbsent)
                {
                    canExecute = true;
                    OnCanExecuteChanged();
                }
